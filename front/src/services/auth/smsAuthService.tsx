...
export async function sendSMSToUser() {
  return await getResponseData<SendSMSToUserRequest, SendSMSToUserResponse>(urls.sendSMSToUser);
}
...
export async function authBySMSCode(smscode: string) {
  const data = await getResponseData<AuthBySMSCodeRequest, AuthBySMSCodeResponse>(
    urls.authBySMSCode,
    { smscode }
  );
...
export async function sendSMSAndSaveNumber(phoneNumber: string) {
  return await getResponseData<SendSMSAndSaveNumberRequest, SendSMSAndSaveNumberResponse>(
...
export async function enableSMSAuth(smscode: string) {
  return await getResponseData<EnableSMSAuthRequest, EnableSMSAuthResponse>(urls.enableSMSAuth, {
    smscode
  });
}
...
export async function disableSMSAuth() {
  return await getResponseData<DisableSMSAuthRequest, DisableSMSAuthResponse>(urls.disableSMSAuth);
}
