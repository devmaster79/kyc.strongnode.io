...
export function SMSSetupModal({ anim, onSuccess, onClose }: SMSSetupModalProps) {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button type="button" variant="medium" onClick={() => verifySMSCode()}>
            Confirm
          </Button>
        </>
      }
...
const SendSMSAndSaveNumberMessage = (props: MessageProps<typeof sendSMSAndSaveNumber>) => {
  if (props.data.result === 'loading') {
    return <DashboardForm.InfoMessage>Sending the SMS...</DashboardForm.InfoMessage>;
  }
  ...
  if (props.data.result === 'validation-error') {
    return <DashboardForm.ErrorMessage>Wrong phone number.</DashboardForm.ErrorMessage>;
  }
  return (
    <DashboardForm.ErrorMessage>
...
const VerifyOTPAndEnableSMSAuthMessage = (props: MessageProps<typeof enableSMSAuth>) => {
  if (props.data.result === 'loading') {
    return <DashboardForm.InfoMessage>Verifying the password...</DashboardForm.InfoMessage>;
  }
  if (props.data.result === 'validation-error') {
    return <DashboardForm.ErrorMessage>Wrong password.</DashboardForm.ErrorMessage>;
  }
  if (props.data.result === 'success') {
...
const SendButtonWithCounter = ({ onClick, disabled }: SendButtonWithCounterProps) => {
