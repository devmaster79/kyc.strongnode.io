...
import { RegistrationService, UserNameIsAlreadyTakenError, LimitReachedError } from '../services/auth/RegistrationService'
...
import { zodValidationError, success, apiResponse } from 'shared/endpoints/responses'
import { sendSMSLimit, authOTPLimit, authPasswordLimit } from '../middleware/limits'
...
export const register =
    withResponse<Register.Response>(async req => {
...
export const enablePasswordAuth =
    withResponse<EnablePasswordAuth.Response>(async req => {
...
export const disablePasswordAuth =
    withResponse<DisablePasswordAuth.Response>(async req => {
        await passwordAuthService.removePassword(req.user.email);
        return success({})
    });

export const authByPassword =
    withResponse<AuthByPassword.Response>(async req => {
        const data = AuthByPassword.schema.parse(req.body)
        const token = await passwordAuthService.authByPassword(req.user.email, data.password);
        if (!token) {
            return zodValidationError([{
                code: "custom",
                path: ["password"],
...
export const sendSMSToUser =
    withResponse<SendSMSToUser.Response>(async req => {
        await smsAuthService.sendSMS(req.user.email);
        return success({})
    });

export const authBySMSCode =
    withResponse<AuthBySMSCode.Response>(async req => {
...
export const sendSMSAndSaveNumber =
    withResponse<SendSMSAndSaveNumber.Response>(async req => {
...
export const enableSMSAuth =
    withResponse<EnableSMSAuth.Response>(async req => {
        const data = EnableSMSAuth.schema.parse(req.body)
        const result = await smsAuthService.activateSMSAuth(req.user.email, data.smscode);
        if (!result) {
            return zodValidationError([{
                code: "custom",
                path: ["smscode"],
...
export const disableSMSAuth =
    withResponse<DisableSMSAuth.Response>(async req => {
        await smsAuthService.deactivate(req.user.email);
        return success({})
    });

export const authByAuthenticator =
    withResponse<AuthByAuthenticator.Response>(async req => {
        const data = AuthByAuthenticator.schema.parse(req.body)
        const token = await authenticatorAuthService.authByAuthenticator(req.user.email, data.token);
        if (!token) return zodValidationError([{
            code: 'custom',
            path: ['token'],
            message: 'Wrong OTP'
        }]);
...
export const enableAuthenticatorAuth =
    withResponse<EnableAuthenticatorAuth.Response>(async req => {
        ...
        const result = await authenticatorAuthService.activateAuthenticatorAuth(req.user.email, data.token);
        if (!result) return zodValidationError([{
            code: 'custom',
            path: ['token'],
            message: 'Wrong OTP'
        }]);
