...
export const addOrUpdateWallet = async (req: UserRequest, res: Response) => {
  ...
  if (!req.body.wallet)
    return res
      .status(400)
      .send({ message: 'Required parameter wallet is not present.' })

  const userDetails = await User.findOne({ where: { email: email } })

  if (userDetails) {
    const userWallets = await UserWallets.findAll({
      where: { user_id: userDetails.id, wallet: req.body.wallet }
    })
...
export const getUserWallets = async (req: UserRequest, res: Response) => {
  ...
  if (userDetails) {
    ...
    return res
      .status(500)
      .send({ message: 'Error getting the wallet data for user.' })
  }
}
